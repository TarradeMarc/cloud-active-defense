...
keycloak_token=$(docker exec controlpanel-api curl -v -X POST http://host.docker.internal:8080/realms/cad/protocol/openid-connect/token -H 'content-type: application/x-www-form-urlencoded' -d 'client_id=cad' -d 'username=test&password=test&grant_type=password')
